...
- Expect: several minutes of wall clock and roughly $3–8 of list cost per run against the $15 cap (20 quarters on `claude-opus-5`, effort high); 1 `list_transcripts` + up to 20 `fetch_transcript` (≤2 in parallel) + 1 `post_artifact` through the webhook, plus sandbox `write`/`read` calls for the notes that never touch the app. Re-measure after the next NVDA run and update this line. If a non-`ended` run already exists for the subject, nothing new starts.
